...
public class ControlProducto extends HttpServlet {
    ...
    protected void EditarDatos(HttpServletRequest request, HttpServletResponse response)
        ...
        c.setIdProducto(Integer.parseInt(request.getParameter("id_producto")));
        c.setNombre(request.getParameter("nombre"));
        c.setPrecio(Double.parseDouble(request.getParameter("precio")));
        c.setStock(Integer.parseInt(request.getParameter("stock")));
        c.setEstadoProducto(request.getParameter("estado"));

        if (obj.BuscarPorNombre(c.getNombre(), c.getIdProducto()) != null) {
            msj = "El producto " + c.getNombre() + " ya se encuentra registrado.";
        } else {
    ...
    protected void GuardarDatos(HttpServletRequest request, HttpServletResponse response)
        ...
        Producto c = new Producto();
        c.setIdProducto(obj.MaximoId());
        c.setNombre(request.getParameter("nombre"));
        c.setPrecio(Double.parseDouble(request.getParameter("precio")));
        c.setStock(Integer.parseInt(request.getParameter("stock")));
        c.setEstadoProducto(request.getParameter("estado"));

        if (obj.BuscarPorNombre(c.getNombre(), 0) != null) {
            msj = "El producto " + c.getNombre() + " ya se encuentra registrado.";
        } else {
            res = obj.Registrar(c);
        }
    ...
    protected void VistaPrincipal(HttpServletRequest request, HttpServletResponse response)
        ...
        String pagina = "PagProductos.jsp";
    ...
    protected void ListarTodos(HttpServletRequest request, HttpServletResponse response)
        ...
        String pagina = "/listado/PagListadoProductos.jsp";

        ArrayList<Producto> lista = obj.Listado();
